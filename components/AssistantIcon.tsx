// The Analytics Assistant's brand mark — the Keel mark.
...
export default function AssistantIcon({
  ...
  if (bare) {
    return (
      ...
      <img src="/mark-white.svg" alt="" width={size} className={`block shrink-0 ${className}`} />
    );
  }
  return (
    <span
    ...
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/mark-white.svg" alt="" style={{ width: Math.round(size * 0.58) }} className="block" />
    </span>
  );
}
