...
import { Folder, FileText, FileImage, FileSpreadsheet, FileType, ExternalLink, Loader2, RefreshCw, ChevronRight, HardDrive, AlertTriangle, UserPlus, FolderTree, Check, X } from 'lucide-react';
import { browseDrive, tagFileToUser, provisionAllDriveFolders } from '@/app/actions';
...
export default function DriveBrowser({ people, canProvision }: { people: Person[]; canProvision: boolean }) {
  const [path, setPath] = useState<Crumb[]>([{ id: undefined, name: 'My Drive' }]);
  const [files, setFiles] = useState<DriveEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [toast, setToast] = useState('');
  const [tagging, setTagging] = useState<DriveEntry | null>(null);
  ...
  const openFolder = (f: DriveEntry) => { setPath((p) => [...p, { id: f.id, name: f.name }]); load(f.id); };
  const goTo = (i: number) => { const next = path.slice(0, i + 1); setPath(next); load(next[next.length - 1].id); };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {canProvision && (
            <button onClick={provision} disabled={provisioning} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50">
              {provisioning ? <Loader2 size={13} className="animate-spin" /> : <FolderTree size={13} />} Create client/project folders
            </button>
          )}
          <button onClick={() => load(current.id)} disabled={loading} className="grid h-8 w-8 place-items-center rounded-md border border-slate-200 text-slate-400 hover:text-slate-700 disabled:opacity-50" title="Refresh">
      ...
      <div className="mt-2 max-h-[28rem] overflow-y-auto rounded-lg border border-slate-100">
        {error ? (
              ...
              return (
                <li key={f.id}>
                  <div className="flex items-center gap-2.5 px-3 py-2 hover:bg-slate-50">
                    ...
                    {!f.isFolder && (
                      <button onClick={() => setTagging(tagging?.id === f.id ? null : f)} title="Tag a teammate" className="shrink-0 text-slate-300 hover:text-brand">
                        <UserPlus size={15} />
                      </button>
                    )}
                    {!f.isFolder && f.webViewLink && <a href={f.webViewLink} target="_blank" rel="noreferrer" className="shrink-0 text-slate-300 hover:text-slate-600"><ExternalLink size={13} /></a>}
                  </div>
                  {tagging?.id === f.id && <TagRow file={f} people={people} onClose={() => setTagging(null)} onDone={flash} />}
                </li>
              );
            })}
          </ul>
      ...
      <p className="mt-2 text-[11px] text-slate-400">
        Live from the connected Google account. Tag a teammate on a file — they get read access + a notification with a direct link.
      </p>
    </div>
  );
}
...
function TagRow({ file, people, onClose, onDone }: { file: DriveEntry; people: Person[]; onClose: () => void; onDone: (m: string) => void }) {
  ...
  const submit = async () => {
    ...
    if (r.ok) {
      ...
      onDone(`Tagged ${who} on “${file.name}”.`);
      onClose();
    } else {
      onDone(r.error ?? 'Could not tag.');
    }
  };
