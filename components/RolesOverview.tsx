...
export default function RolesOverview({
  ...
  // A non-super viewer (Admin) never sees Super Admin as a distinct type — it's
  // folded into "Admin" everywhere, so hide its column/card entirely here.
  const roles = viewerIsSuper ? ROLE_ORDER : ROLE_ORDER.filter((r) => r !== 'SUPER_ADMIN');
            ...
            {groups.map((g) => (
              <Fragment key={g}>
                <tr className="bg-slate-50/60">
                  <td colSpan={roles.length + 1} className="px-1 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{g}</td>
                </tr>
